                ...
                <!-- 顶级菜单 -->
                    <c:forEach var="m"  items="${menus}">
                       <c:if test="${m.parentId==0}">
                          ...
                          <!-- 左侧菜单 -->
                                     <h2>${m.menuName}<i></i></h2>
                                     <ul style="display: none;">
                                     <!-- 左侧一级子菜单 -->
                                       <c:forEach var="n"  items="${menus}">
                                         <c:if test="${n.parentId==m.id}">
                                           <li>
                                             <a navid="channel_news" class="item pack">
                                                <span>${n.menuName}</span>
                                             </a>
                                             <ul style="display: block;">
                                             <!-- 左侧二级子菜单 -->
                                                <c:forEach var="u"  items="${menus}">
                                                  <c:if test="${u.parentId==n.id}">
                                                    <li>
                                                      <a navid="channel_news_list" href="${u.menuUrl}" target="mainframe" class="item">
                                                         <span>${u.menuName}</span>
                                                      </a>
                                                    </li>
                                                  </c:if>
                                                </c:forEach>
                    ...
                    <script type="text/javascript">
                     $(function(){
                         ...
                         //自身添加样式
                         //$(this).addClass("selected");
                         frames["mainframe"].location.href = '/views/center.jsp';
                     })
                    </script>
                </div>
            </div>
